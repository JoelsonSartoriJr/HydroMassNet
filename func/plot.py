...
# Set global parameters for plot aesthetics
plt.rcParams.update({
    'font.size': 16,
    'figure.figsize': (12, 6),  # consistent figure size
    'lines.linewidth': 2,       # thicker lines for better visibility
    'axes.grid': True           # enable grid by default for clarity
})
...
def make_predictions_and_plot_residuals(model1, model2, data_val, scaler_y_fit):
    for x_data, y_data in data_val:
        ...
        plt.xlabel(r'True values ($log(M_{\odot})$)')
        plt.ylabel(r'Predict values ($log(M_{\odot})$)')
...
def plot_predictive_distributions(model1, model2, data_val, scaler_y_fit):
    for x_data, y_data in data_val:
        ...
        # Gráfico para BNN
        plt.figure()
        plt.hist(preds1_real, bins=50, alpha=0.5, label='BNN', color='blue')
        plt.hist(y_val_real, bins=50, alpha=0.5, label='True', color='red')
        ...
        # Gráfico para DBNN
        plt.figure()
        plt.hist(preds2_real, bins=50, alpha=0.5, label='DBNN', color='green')
        plt.hist(y_val_real, bins=50, alpha=0.5, label='True', color='red')
        plt.xlabel('Predicted values')
        plt.ylabel('Density')
        plt.legend()
