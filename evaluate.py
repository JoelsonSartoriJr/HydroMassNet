...
def main(model_name: str, config_path: str = "config.yaml"):
    """
    ...
    if model_cfg["type"] in ["bnn", "dbnn"]:
        ...
        y_pred_samples = [
            model(x_test, training=True).numpy() for _ in tqdm(range(n_samples))
        ]
    ...
    y_pred_mean_real = data_handler.scaler_y.inverse_transform(
